...
@app.post(
    "/api/v1/remix",
    response_model=IngestResponse,
    summary="Remix stems into a new stitched fragment",
    description="Takes isolated stems from a parent fragment, merges them into a single track natively via ffmpeg, and branches it as a new stitched fragment.",
)
async def remix_fragment(req: RemixRequest):
    """
    1. Fetch parent fragment matching parent_id.
    2. Download/stream the selected HTTP stems into ffmpeg amix.
    3. Export single fused wav.
    4. Upload to Supabase as a new stitched fragment.
    """
    from app.supabase_client import get_supabase
    
    if not req.stems:
        raise HTTPException(status_code=400, detail="No stems selected for remix.")

    client = get_supabase()
    if not client:
        raise HTTPException(status_code=503, detail="Database not configured.")

    # 1. Fetch parent fragment
    res = client.table("fragments").select("*").eq("id", req.parent_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Parent fragment not found.")
    parent = res.data[0]

    parent_stems = parent.get("stem_urls", {})
    if not parent_stems:
        raise HTTPException(status_code=400, detail="Parent has no stems attached.")

    selected_urls = []
    for s in req.stems:
        if s in parent_stems:
            selected_urls.append(parent_stems[s])
        else:
            logger.warning(f"  ⚠ Requested stem '{s}' not found on parent. Skipping.")

    if not selected_urls:
        raise HTTPException(status_code=400, detail="None of the selected stems exist in the parent stem registry.")

    fragment_id = str(uuid.uuid4())
    tmp_path = os.path.join(tempfile.gettempdir(), f"remix_{fragment_id}.wav")

    # 2. Build FFMpeg command
    ffmpeg_cmd = [_get_ffmpeg_path(), "-y"]
    for url in selected_urls:
        ffmpeg_cmd.extend(["-i", url])

    if len(selected_urls) == 1:
        # Just copy/convert format
        ffmpeg_cmd.extend(["-c:a", "pcm_s16le", "-ar", "22050", "-ac", "2", tmp_path])
    else:
        # Mix multiple
        ffmpeg_cmd.extend([
            "-filter_complex", f"amix=inputs={len(selected_urls)}:duration=longest:dropout_transition=0",
            "-c:a", "pcm_s16le", "-ar", "22050", "-ac", "2", tmp_path
        ])

    logger.info(f"🎛️ Remixing {len(selected_urls)} stems... CMD: {' '.join(ffmpeg_cmd)}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Remix operation timed out.")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

    # 3. Upload mixed WAV to Supabase Storage
    try:
        with open(tmp_path, "rb") as f:
            file_content = f.read()
    except FileNotFoundError:
        raise HTTPException(status_code=500, detail="Output file was not generated by FFmpeg.")

    storage_path = f"fragments/{fragment_id}_remix.wav"
    settings = get_settings()
    base_public_url = f"{settings.supabase_url}/storage/v1/object/public/ovo_audio"
    ...
    # 4. Insert DB record
    new_title = f"{parent.get('title', 'Untitled')} — Remix"
    ...
    db_record = {
        ...
        "mood": parent.get("mood", "Mixed"),
        "title": new_title,
        "file_url": file_url,
        "stem_urls": {}, # Stitched files don't automatically stem split themselves immediately
    }

    # Inherit parent's embedding if present
    if "embedding" in parent and parent["embedding"]:
        db_record["embedding"] = parent["embedding"]

    client.table("fragments").insert(db_record).execute()
    logger.info(f"💾 Saved Remix DB Fragment: {fragment_id}")
    
    from app.models import FragmentResponse, IngestResponse
    ...
    return IngestResponse(
        success=True,
        fragment=fragment_response,
        message="Stems successfully mixed and saved to active branch."
    )
